...
Before placing any elements, decide your target output dimensions. This determines your coordinate space — all element `x`/`y` positions must fit within these bounds.

| Preset | Aspect Ratio | Canvas Size (px) | Use For |
|--------|-------------|-------------------|---------|
| LinkedIn Carousel | 4:5 portrait | 1080 × 1350 | Carousel slides, social media images |
| Presentation | 16:9 landscape | 1920 × 1080 | Slide decks, video thumbnails |
| Square | 1:1 | 1080 × 1080 | Social media posts |
| Auto | (none) | Fits content | Documentation, README diagrams |

**How to use:** When a target aspect ratio is specified, compose your diagram within the canvas bounds. Place elements in the range `x: 40..{width-40}`, `y: 40..{height-40}` (40px internal margin). The renderer's `--aspect` flag will enforce the output ratio with padding if needed.

**Default:** If no aspect ratio is specified, the renderer auto-sizes from the content bounding box (the "Auto" preset). **This is the recommended approach for most diagrams** — it produces the most reliable results.

**`--aspect` gotcha:** The renderer's `--aspect` flag clips the page screenshot to the computed viewport, but Excalidraw's `exportToSvg` adds its own internal padding that may push the SVG beyond the viewport bounds. This causes content to be cut off on the right or bottom edge. **If content is clipped when using `--aspect`, render without it** and let the auto-sizing handle dimensions. Only use `--aspect` when you've verified the SVG fits within the computed viewport.

**Render command with aspect ratio:**
```bash
cd .claude/skills/excalidraw-diagram/references && uv run python render_excalidraw.py <file> --aspect 4:5
```
...
### Start Compact
Target a content width of **800–1000px** for pipeline/flow diagrams. You can always spread out if there's room, but starting wide (1200px+) almost always leads to clipping or rewriting. For N elements in a row, use roughly `N × 180px` as a starting width estimate (element width + gap).

### Decide Arrow Types Up Front
Before writing any JSON, define your arrow vocabulary (typically 2–3 types). Each type should differ in exactly one visual property — color, dash style, or stroke width. Example: workflow (solid dark), artifact-link (dashed light), error (dashed red). Matching arrow color to each source element creates visual noise.
...
**6. Repeat** — Keep cycling until the diagram passes both the vision check (Step 2) and the defect check (Step 3). **Minimum 2 render passes per diagram.** The first render is always a draft — find at least one thing to improve before declaring it done. Typically takes 2-4 iterations total. Don't stop after one pass just because there are no critical bugs — if the composition could be better, improve it.
