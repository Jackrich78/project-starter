...
Usage:
    ...
    uv run python render_excalidraw.py <path-to-file.excalidraw> [--output path.png] [--scale 2] [--width 1920] [--aspect 4:5]
...
def parse_aspect(aspect_str: str | None) -> tuple[int, int] | None:
    """Parse aspect ratio string like '4:5' into (w, h) tuple."""
    if not aspect_str:
        return None
    parts = aspect_str.split(":")
    if len(parts) != 2:
        print(f"ERROR: Invalid aspect ratio '{aspect_str}'. Use format like '4:5' or '16:9'.", file=sys.stderr)
        sys.exit(1)
    try:
        return (int(parts[0]), int(parts[1]))
    except ValueError:
        print(f"ERROR: Invalid aspect ratio '{aspect_str}'. Use integers like '4:5'.", file=sys.stderr)
        sys.exit(1)
...
def render(
    ...
    aspect_ratio = parse_aspect(aspect)
    if aspect_ratio:
        # Enforce aspect ratio: use content width, compute height from ratio
        aw, ah = aspect_ratio
        vp_width = min(int(diagram_w), max_width)
        target_height = int(vp_width * ah / aw)
        # If content is taller than the target, use content height and widen
        if diagram_h > target_height:
            vp_height = max(int(diagram_h), 600)
            vp_width = int(vp_height * aw / ah)
        else:
            vp_height = target_height
    else:
    ...
    with sync_playwright() as p:
        ...
        if aspect_ratio:
            # Full-page screenshot at the enforced viewport dimensions
            page.screenshot(path=str(output_path), clip={"x": 0, "y": 0, "width": vp_width, "height": vp_height})
        else:
            # Content-sized screenshot (original behavior)
...
def main() -> None:
    ...
    parser.add_argument("--aspect", "-a", type=str, default=None, help="Target aspect ratio, e.g. '4:5' or '16:9'")
    ...
    if args.input.resolve().suffix not in ('.excalidraw', '.json'):
        print(f"ERROR: Expected .excalidraw or .json file, got '{args.input.suffix}'", file=sys.stderr)
        sys.exit(1)

    png_path = render(args.input, args.output, args.scale, args.width, args.aspect)
    print(str(png_path))
